    ...
    start_server {tags {"dual-channel-replication external:skip"}} {
        ...
        start_server {} {
            ...
            for {set j 0} {$j < 50} {incr j} {
                $primary set key $bigstr
                lappend write_times [expr {[clock milliseconds] - $t_writes_start}]
            }
            ...
            for {set i 0} {$i < 1000} {incr i} {
                ...
                lappend trace "$i@[expr {[clock milliseconds] - $t_writes_end}] p=$p_tot r=$r_tot l=$r_len s=$r_sip/$r_lnk"
                if {$p_tot < [expr 1024000 * 10] && $r_tot > [expr 1024000 * 40]} {
                    set met 1
                    break
                }
                after 50
            ...
            if {!$met} {
                ...
                for {set k 0} {$k < $n} {incr k} {
                    if {$k % 25 == 0 || $k >= $n - 10} { lappend sampled [lindex $trace $k] }
                }
                fail "replica didn't receive the data in time | write_ms=$write_ms write_times=[join $write_times ,] | primary_replicas=[$primary client list type replica] | TRACE [join $sampled { || }]"
            }
            ...
            if {[getInfoProperty $replica_info replicas_repl_buffer_size] < [expr 1024000 * 40]} {
                fail "SIGB gate=$probe_met_at snapshot: len=[getInfoProperty $replica_info replicas_repl_buffer_size] mem=[getInfoProperty $replica_info mem_replicas_repl_buffer] tot=[getInfoProperty $replica_info mem_total_replication_buffers] peak=[getInfoProperty $replica_info replicas_repl_buffer_peak]"
            }
